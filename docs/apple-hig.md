...
| Rule | For a web app on iPhone | Deka today | Source |
|---|---|---|---|
| Do not use one color to mean different things. Keep status and interactive colors consistent. | Reserve the done color for done. Do not reuse it for links or decoration. | Yes. Chartreuse means done and only done: done sessions, done log results, completed goals and the check off burst. Today is an ink ring, the primary button is solid ink, the selected tab is a neutral lens, and selection is an ink tint. | https://developer.apple.com/design/human-interface-guidelines/color |
...
| Prefer filled symbols or icons for tab bar icons. | Use filled Phosphor icons, at least for the selected tab. | Partly. Selection shows as full ink and a neutral glass lens; the icons stay outlines. | https://developer.apple.com/design/human-interface-guidelines/tab-bars |
...
| Avoid a tab label color similar to content backgrounds. With colorful content, keep the tab bar monochrome. | Keep tab icons in ink; mark selection with weight or fill, and the accent only if it contrasts. | Icons are soft ink, full ink when selected under a neutral lens. No accent color in the tab bar. | https://developer.apple.com/design/human-interface-guidelines/tab-bars |
...
| Detents: large is fully expanded; medium is about half of the full height. Sheets support large by default. Only medium prevents full height. | Implement two rest heights: about half and full. | Tall sheets open at a medium detent, 60% of the screen, and drag up to large. The goal sheet opens at large, so every field is in view. Short sheets fit their content. | https://developer.apple.com/design/human-interface-guidelines/sheets |
| On iPhone, consider the medium detent for progressive disclosure. Skip it when content needs full height, like compose sheets. | Same. | The goal sheets, edit and new, open at large; a text field in any other tall sheet takes it to large. | https://developer.apple.com/design/human-interface-guidelines/sheets |
...
| Use a prominent style for the most likely action. Keep prominent buttons to one or two per view. | One filled primary button per view. | Yes. One solid ink primary per view, such as Confirm or Add (ivory with ink text in dark). | https://developer.apple.com/design/human-interface-guidelines/buttons |
