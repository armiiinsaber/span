...
function plan(params) {
  ...
  const files = Array.isArray(last.content) ? last.content.filter(b => b.type === 'image' || b.type === 'document') : [];
  ...
  const body = Array.isArray(last.content) ? last.content.filter(b => b.type === 'text').map(b => b.text).join('\n\n') : last.content;
  ...
  // Files: the mock cannot read them, so it says what arrived.
  if (files.length && !said) {
    const n = files.filter(b => b.type === 'image').length, docs = files.length - n;
    return { text: `Got ${[n ? `${n} ${n === 1 ? 'photo' : 'photos'}` : '', docs ? `${docs} ${docs === 1 ? 'PDF' : 'PDFs'}` : ''].filter(Boolean).join(' and ')}. Mock read, not Claude.`, tools: summary };
  }
  ...
  if (found.length) {
    ...
    // Too much for nine days: suggest a third off the biggest goals, the way Claude would.
    const total = found.reduce((n, g) => n + g.target, 0) + (ctx.goals || []).filter(g => !found.some(f => f.id === g.id)).reduce((n, g) => n + g.target, 0);
    const big = found.filter(g => g.target >= 5);
    if (total > 30 && big.length) {
      tools.push(tool('suggest_trim', { trims: big.map(g => ({ goal_id: g.id, requested: g.target, suggested: Math.ceil(g.target * 0.65) })), reason: 'too full for nine days' }));
      return { text: 'Too full for nine days. Here is a lighter version.', tools: [...summary, ...tools] };
    }
  ...
  // An open trim answered in words: the same choice as the buttons, then a plan.
  const accept = /sounds good|do that|go with (it|that)|use it|fine by me/.test(lower), decline = /keep my (numbers|counts)|keep mine|no trim/.test(lower);
  if (ctx.open_trim && (accept || decline)) {
    const choice = decline ? 'keep' : 'use';
    tools.push(tool('resolve_trim', { choice }));
    const goals = ctx.goals.map(g => { const t = choice === 'use' && ctx.open_trim.trims.find(x => x.goal_id === g.id); return t ? { ...g, target: t.suggested } : { ...g }; });
    tools.push(tool('propose_schedule', { occurrences: schedule(ctx, goals), summary: choice === 'use' ? 'The lighter plan' : 'Your numbers, squeezed in' }));
    return { text: choice === 'use' ? 'Here is the lighter plan.' : 'Here are your numbers. The evenings get tight.', tools: [...summary, ...tools] };
  }

  if ((ctx.goals || []).length && /plan|schedule|lay|spread|go ahead|yes|free|move|tweak|sure|suggestion|keep mine/.test(lower)) {
