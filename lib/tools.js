...
const TOOLS = [
  {
    name: 'update_goals',
    description: 'Add, edit or remove goals for the deka. Goals are what the person wants done in these 10 days, each with a target count. Use op "add" with a new id, "edit" with an existing id (empty name, "unchanged" type and target 0 keep the old value), or "remove" with an existing id.',
    strict: true,
    input_schema: {
      ...
      properties: {
        changes: {
          ...
          items: {
            ...
            required: ['op', 'id', 'name', 'type', 'tag', 'target'],
            properties: {
  ...
  {
    name: 'log_day',
    description: 'Mark what got done or missed on one day that has already started. Use it during check ins, only for a day with at least one entry. Marking done something that was planned for a later day moves one of those later sessions to this day.',
    strict: true,
    input_schema: {
  ...
  },
  {
    ...
    strict: true,
    input_schema: {
      type: 'object', additionalProperties: false, required: ['occurrences', 'changes'],
      properties: {
        occurrences: {
          type: 'array',
          items: {
        ...
        changes: {
          type: 'array',
          description: 'Two to six short lines saying what this proposal changes, like "Gym moves from Day 4 to Day 6" or "Added 2 runs on Days 3 and 7".',
          items: { type: 'string' },
        },
      },
    },
...
function working(state) {
  return {
    ...
    goals: (state.goals || []).map(g => ({ id: String(g.id), name: String(g.name), type: g.type, tag: g.tag || '', target: g.target })),
...
function updateGoals(input, w) {
  ...
  for (const [i, c] of changes.entries()) {
    ...
    if (c.op === 'add') {
      ...
      const g = { id, name: name.slice(0, 40), type: c.type, tag: cleanText(c.tag).slice(0, 24), target: c.target };
      next.goals.push(g);
      out.push({ op: 'add', ...g });
    } else if (c.op === 'edit') {
  ...
  return { ok: true, event: { type: 'goals', changes: out }, result: `Saved. Goals now: ${w.goals.map(g => `${g.name} (${g.id}) x${g.target}`).join(', ') || 'none'}. If your reply is written and you have made every call you need, end the turn with no more text.` };
}
...
function logDay(input, w) {
  ...
  return {
    ...
    result: `Logged day ${day}: ${applied.map(a => `${a.goal_id} ${a.status}${a.moved_from ? ` (moved from day ${a.moved_from})` : ''}`).join(', ') || 'nothing'}.${skipped.length ? ` Skipped: ${skipped.join('; ')}.` : ''} If your reply is written and you have made every call you need, end the turn with no more text.`,
  };
}
...
function proposeSchedule(input, w) {
  ...
  const changes = (Array.isArray(input.changes) ? input.changes : []).map(cleanText).filter(Boolean).slice(0, 8);
  if (!changes.length) errors.push('changes must list what this proposal changes, in short lines.');
  if (errors.length) return { ok: false, errors };
  ...
  return {
    ok: true,
    event: { type: 'proposal', occurrences: pending, changes, changed_days: changedDays },
    result: 'Shown to the person as a card with Confirm and Tweak. It is not applied until they confirm. Do not repeat the changes; if your reply is written, end the turn with no more text.',
  };
}
...
const HANDLERS = { update_goals: updateGoals, log_day: logDay, propose_schedule: proposeSchedule, save_summary: saveSummary };
...
module.exports = { TOOLS, SUMMARY_TOOL, working, runTool, WORK_DAYS };
