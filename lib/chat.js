...
Attachments
They can send photos, screenshots and PDFs with a message. Use them for planning: a photo of a to do list, a screenshot of a calendar or schedule, a syllabus, a workout plan. Read what is there and turn it into goals or plan changes through the normal tools, exactly as if they had typed it. Items on a to do list become goals, a one off task with a target of 1. Fixed commitments on a calendar are limits the plan works around, not goals. From a syllabus or a workout plan, take what falls in these ten days. The goals card shows what you saved, so your reply never names the items back, not even in a short list; say only the one thing that matters, like whether it all fits. Ask one short question only if something is unreadable or truly unclear. An attachment with nothing to do with their plan, like a landscape, a meme or a document about something else, gets the usual one short line back to their deka, two sentences at most and no question, and a call to mark_off_topic, nothing more. Earlier attachments show in the history only as a note like [Attached a photo]; you cannot see them again.
...
// Photos go to Claude as image blocks and PDFs as document blocks, at most 5 with a message.
const IMAGE_TYPES = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
const MAX_ATTACHMENTS = 5;
const MAX_PDF = 3 * 1024 * 1024;
const B64 = /^[A-Za-z0-9+/]+={0,2}$/;
function checkAttachments(list) {
  if (list == null) return { ok: true, list: [] };
  const bad = { ok: false, status: 400, error: 'Those attachments cannot be sent.' };
  if (!Array.isArray(list)) return bad;
  if (list.length > MAX_ATTACHMENTS) return { ok: false, status: 400, error: 'Up to 5 attachments per message.' };
  for (const a of list) {
    if (!a || typeof a.data !== 'string' || !B64.test(a.data)) return bad;
    const bytes = Math.floor(a.data.length * 3 / 4);
    if (a.media_type === 'application/pdf') { if (bytes > MAX_PDF) return { ok: false, status: 413, error: 'That PDF is over 3 MB.' }; }
    else if (!IMAGE_TYPES.includes(a.media_type)) return bad;
  }
  return { ok: true, list };
}
const attachmentBlocks = list => (list || []).map(a => a.media_type === 'application/pdf'
  ? { type: 'document', source: { type: 'base64', media_type: 'application/pdf', data: a.data }, ...(a.name ? { title: clip(String(a.name), 200) } : {}) }
...
function buildMessages(body) {
  ...
  const files = attachmentBlocks(checkAttachments(body.attachments).list);
  const said = body.message ? `Person: ${clip(body.message, 4000)}` : files.length ? 'Person sent the attachments above with no message.' : '';
  const now = [contextBlock(body), said].filter(Boolean).join('\n\n');
  const last = turns[turns.length - 1];
  if (!files.length) {
    if (last && last.role === 'user') last.content += `\n\n${now}`;
    else turns.push({ role: 'user', content: now });
  } else if (last && last.role === 'user') {
    last.content = [{ type: 'text', text: last.content }, ...files, { type: 'text', text: now }];
  } else turns.push({ role: 'user', content: [...files, { type: 'text', text: now }] });
...
module.exports = { runChat, buildMessages, contextBlock, checkAttachments, SYSTEM_PROMPT, MODEL, EFFORT, CEILING, MAX_ROUNDS };
