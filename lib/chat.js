...
const { TOOLS, STATUS_TOOL, SUMMARY_TOOL, working, runTool } = require('./tools');
    ...
    for (const use of uses) {
      ...
      if (r.ok && use.name === 'propose_schedule') {
        ...
        if (fresh.length && !scoreTried) {
          ...
          results.push({ type: 'tool_result', tool_use_id: use.id, is_error: true, content: `Valid, but it can be better. Fix these and call propose_schedule again with the whole plan:\n${fresh.map(f => f.text).join('\n')}` });
          continue;
        }
        if (waiting) {
