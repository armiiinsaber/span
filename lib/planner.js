...
Review
In review mode, the plan state is the span that just ended. Write the review field: three or four short sentences. Say what held, what slipped, and one pattern worth noticing. Be honest and kind, never gushing. Then draft the next span's intentions and occurrences, using the next span's day mapping, based on what the person can realistically carry. The note says in one line how the draft differs from last time.
...
const SET_PLAN_TOOL = {
  name: 'set_plan',
  description: 'Return the full plan for the span: every intention and every occurrence, plus a short note to the person. Always call this, even when asking a question.',
  strict: true,
  input_schema: {
    type: 'object',
    ...
    properties: {
      intentions: {
        ...
        items: {
          ...
          properties: {
            ...
            target: { type: 'integer', description: 'How many times in the span. 1 to 9.' },
          },
        },
      },
      occurrences: {
      ...
      review: { type: 'string', description: 'Only in review mode: the honest read of the span. Otherwise empty string.' },
    },
  },
};
...
function buildUserMessage(body) {
  ...
  if (body.currentDay) lines.push(`Today is day ${body.currentDay} of the current span.`);
  lines.push('');
  lines.push(body.mode === 'review' ? 'Next span day mapping:' : 'Span day mapping:');
  ...
  if (body.plan && (body.plan.intentions || []).length) {
    lines.push('');
    lines.push(body.mode === 'review' ? 'The span that just ended:' : 'Current plan:');
  ...
  if ((body.history || []).length) {
    lines.push('');
    lines.push('Conversation so far this span:');
    for (const h of body.history) lines.push(`${h.role === 'user' ? 'Person' : 'You'}: ${h.text}`);
  }
  if ((body.past || []).length) {
    lines.push('');
    lines.push('Recent spans, newest first:');
